...
build_dynamic_cast_node (void) {
  ...
  tree dcast_fn = (build_library_fn_ptr
  ...
  if (TYPE_POLYMORPHIC_P (TREE_TYPE (exprtype)))
      ...
      else
	{
	  ...
	  if (flag_assume_unique_vtables && CLASSTYPE_FINAL (target_type) &&
	    ...
	    tree binfo =
	      lookup_base (target_type, static_type, ba_check, NULL, complain);
	    if (!binfo || binfo == error_mark_node)
	      return error_mark_node;

	    tree target_vtbl = build_vtbl_address (
		BINFO_VTABLE (binfo) ? TYPE_BINFO (target_type) : binfo);
	    tree expr_vtbl =
	      build_vfield_ref (cp_build_fold_indirect_ref (expr), static_type);
	    ...
	    result =
		build3 (COND_EXPR, ptr_type_node, cmp, converted, nullptr_node);
	  } else {
